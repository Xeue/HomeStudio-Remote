...
templates.encoder = `<% for(i = 0; i < devices.length; i++) { %>
  <tr data-index="<%=i%>" data-id="<%-devices[i].ID%>" data-template="encoder">
	...
	<td data-type="select" data-key="Type" data-value="<%-devices[i].Type%>" data-options="Select,SDI,SRT,WebSources"><%-devices[i].Type%></td>
...
$(document).ready(function() {
	...
	localConnection.addEventListener('open', () => {
		...
		$('#broken').html(`<span class="p-2 bg-success">${currentSystem} Online</span>`);
	});
	localConnection.addEventListener('close', () => {
		$('#broken').html(`<span class="p-2 bg-danger">${currentSystem} Offline</span>`);
	});
	...
	setInterval(() => {
		for (_thumb of document.getElementsByClassName('thumbnail')) {
			if (_thumb.parentNode.getAttribute('data-type') == "WebSources") continue;
